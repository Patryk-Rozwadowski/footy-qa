...
const BASE_URL = 'https://stacy.olympics.com/srm/data/oly/schedule/day/ENG'
const HEADERS = {
  'User-Agent':
...
async function main() {
  ...
  console.log(`Fetching ${dates.length} days from official API…\n`)
  ...
  const dayResults = await Promise.all(
    dates.map(async (date) => {
      try {
        ...
        console.log(`  ${date}  ${units.length} match(es)`)
        return units
      } catch (err) {
  ...
  const matches = dayResults.flat().flatMap((unit) => {
    ...
    return [{
      ...
      competition: { name: 'Paris 2024 Olympics', season: '2024', round },
  ...
  console.log(`\nTotal: ${matches.length} football matches`)
